...
- `/list` — единый список WireGuard и VLESS/Reality. WireGuard проверяется
  ICMP-пингом на внутренний IP каждого клиента (а не по факту последнего
  handshake, который может быть устаревшим); все клиенты пингуются параллельно.
  VLESS/Reality не выдаёт клиентам внутренние IP, поэтому для него статус и ping
  честно выводятся как «не определяются»
